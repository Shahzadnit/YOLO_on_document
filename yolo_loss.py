...
class YoloLoss(nn.Module):
    ...
    def find_best_iou_boxes(self, box_target, box_pred):
        ...
        for i in range(0, box_target.size()[0],2):
            ...
            box1_xywh[:,:2] = box1[:,:2]/S -0.5*box1[:,2:4] # x, y = x/S - 0.5*w, y/S - 0.5*h
            box1_xywh[:,2:4] = box1[:,:2]/S + 0.5*box1[:,2:4] # w, h = x/S + 0.5*w, y/S + 0.5*h
            box2 = box_target[i].view(-1,5)
            box2_xywh = Variable(torch.FloatTensor(box2.size()))
            box2_xywh[:,:2] = box2[:,:2]/S - 0.5*box2[:,2:4]
            box2_xywh[:,2:4] = box2[:,:2]/S +0.5*box2[:,2:4]

            #1. find iou
            iou = self.compute_iou(box1_xywh[:,:4],box2_xywh[:,:4]) #[2,1]
            # best iou is maximum iou
            maximum_iou, maximum_iou_index = iou.max(0)
            maximum_iou_index = maximum_iou_index.data.cuda()
            # 2. set corresponding contains_object_response_mask to 1
            contains_object_response_mask[i + maximum_iou_index] = 1
